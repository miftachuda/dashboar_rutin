...
export default function UnitDashboardPage() {
  ...
  const { lowStockItems, loading: consumablesLoading } =
    useLowStockConsumables();
  const {
    data: limsData,
    limit: limsLimit,
    loading: limsLoading,
          ...
          {loading ? (
            ...
            <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              ...
              {(() => {
                ...
                if (hasAnyRedundanN0) {
                  glowClass =
                    "bg-red-50 border-red-400 shadow-[0_0_15px_rgba(239,68,68,0.5)] hover:shadow-[0_0_20px_rgba(239,68,68,0.7)]";
                }
                      ...
                      <div className="mt-auto w-full pt-4">
                        <div className="flex w-full flex-col gap-1.5 border-t border-slate-200/60 pt-3 text-left">
                          <div className="flex flex-wrap gap-1.5">
                            {onStreamCount > 0 && (
                              <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-700 shadow-sm">
                                On Stream: {onStreamCount}
                              </span>
                            )}
                            {pitStopCount > 0 && (
                              <span className="rounded bg-sky-100 px-1.5 py-0.5 text-[10px] font-semibold text-sky-700 shadow-sm">
                                Pit Stop: {pitStopCount}
                              </span>
                            )}
                            {turnAroundCount > 0 && (
                              <span className="rounded bg-purple-100 px-1.5 py-0.5 text-[10px] font-semibold text-purple-700 shadow-sm">
                                TA: {turnAroundCount}
                              </span>
                            )}
                          </div>
                  ...
                  >
                    <div className="flex w-full items-start justify-between gap-2">
                      ...
                      <div className="flex items-baseline gap-1.5 text-5xl font-bold leading-none">
                        ...
                        <span className="text-3xl text-slate-300">/</span>
                        <span
                          className={`${color.background} bg-clip-text text-transparent`}
                        >
                          {count}
                        </span>
                      </div>
        ...
        <div className="flex flex-col xl:flex-row w-full gap-6">
          ...
          <section id="chemical" className="w-full xl:w-1/2 flex flex-col">
            ...
            {(() => {
                ...
                return (
                  <div className="rounded-3xl border border-emerald-100 bg-emerald-50/50 p-6 flex flex-col items-center justify-center text-center gap-2 h-full">
                    ...
                    <p className="text-xs text-emerald-600">
                      All monitored chemical vessels are above 30%.
                    </p>
                  </div>
                );
              }
              ...
              return (
                <div className="grid w-full grid-cols-1 sm:grid-cols-2 xl:grid-cols-1 gap-3">
                  {lowChemicals.map((chem) => {
                    const unitColor = getUnitColorPalette(chem.unit);
                    return (
                      <div
                        key={chem.tagName}
                        className="rounded-3xl border p-4 shadow-sm flex flex-col gap-2 bg-orange-50 border-orange-200 relative overflow-hidden group hover:border-orange-300 transition-colors"
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <span
                            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide ${unitColor.background} text-white shadow-sm`}
                          >
                            Unit {chem.unit}
                          </span>
                        </div>
                        <div className="mt-1 flex items-end justify-between">
                          ...
                          <div className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-200/50 px-2 py-1 rounded-md">
          ...
          <section id="tanki" className="w-full xl:w-1/2 flex flex-col">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full border border-sky-200 bg-sky-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-sky-700">
                  Live: {liveTankCount} /{" "}
                  {Object.keys(rowsByTank).length || predefinedTankList.length}
                </span>
              </div>
            </div>
            ...
            {globalCriticalTanks.length === 0 ? (
              <div className="rounded-3xl border border-emerald-100 bg-emerald-50/50 p-6 flex flex-col items-center justify-center text-center gap-2">
                ...
                <p className="text-xs text-emerald-600">
                  No tanks are projected to hit High/Low targets within the next
                  12 hours.
                </p>
              </div>
            ) : (
              <div className="grid w-full grid-cols-1 sm:grid-cols-2 gap-3">
                {globalCriticalTanks.map((ct) => (
                  <div
                    key={ct.tank}
                    className="flex flex-col gap-2 p-4 rounded-2xl bg-orange-50 border border-orange-200 shadow-sm relative overflow-hidden group hover:border-orange-300 transition-colors"
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <span className="text-[11px] font-bold text-orange-700 bg-orange-200/50 px-2 py-0.5 rounded-md">
                        {ct.text}
                      </span>
                    </div>

                    {ct.level !== null && ct.target !== null && (
                      <div className="flex items-center justify-between text-xs font-semibold text-orange-800 mt-1">
                        <span>
                          Level:{" "}
                    ...
                    <div className="mt-2 flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-200/50 px-2 py-1 rounded-md w-fit">
                      <AlertTriangle size={12} />
                      Critical Tank
                    </div>
                  </div>
        ...
        <section id="material" className="w-full">
          ...
          {consumablesLoading ? (
            ...
            <div className="rounded-3xl border border-emerald-100 bg-emerald-50/50 p-6 flex flex-col items-center justify-center text-center gap-2">
              ...
              <p className="text-xs text-emerald-600">
                No consumable materials are below their minimum threshold.
              </p>
            </div>
          ) : (
