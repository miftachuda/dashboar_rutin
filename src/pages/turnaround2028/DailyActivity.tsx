...
export default function DailyActivity() {
  ...
  const deleteActivity = async (activityId: string) => {
    try {
      await pb.collection("daily_activity_ta2028").delete(activityId);
  ...
  async function loadActivity() {
    try {
      const [activityRecords] = await Promise.all([
        pb.collection("daily_activity_ta2028").getFullList({ sort: "-created" }), // example
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (saving) return;
      setSaving(true);
      await pb.collection("daily_activity_ta2028").create(data);
  ...
  const handleStatusChange = async (id: string, newStatus: Status) => {
    try {
      ...
      await pb.collection("daily_activity_ta2028").update(id, {
        status: newStatus,
      });
