...
export default function AdditionalsJoblist() {
  ...
  const handleStepToggle = async (
    ...
    // Sync to PocketBase
    try {
      await pb.collection("additionals_ta2028").update(taskId, {
        steps: updatedSteps,
  ...
  const syncTaskToPocketBase = async (task) => {
    try {
      await pb.collection("additionals_ta2028").update(task.id, {
        steps: task.steps,
        updatedCustom: new Date().toISOString(),
      });
    } catch (err) {
  ...
  async function loadTasks() {
    try {
      const pitstopRecords = await pb.collection("additionals_ta2028").getFullList({
        sort: sortOption,
      });
