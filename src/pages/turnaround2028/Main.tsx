...
const MainPage = () => {
  ...
  const handleStepToggle = async (
    ...
    try {
      await pb.collection("turnaround2028").update(taskId, {
        steps: updatedSteps,
  ...
  const syncTaskToPocketBase = async (task: StepTask) => {
    try {
      await pb.collection("turnaround2028").update(task.id, {
        steps: task.steps,
        updatedCustom: new Date().toISOString(),
      });
    } catch (err) {
  ...
  const handleDeleteJoblist = async (taskId: string) => {
    try {
      await pb.collection("turnaround2028").update(taskId, { isDeleted: true });
  ...
  async function loadTasks() {
    try {
      const pitstopRecords = await pb.collection("turnaround2028").getFullList({
        sort: sortOption,
