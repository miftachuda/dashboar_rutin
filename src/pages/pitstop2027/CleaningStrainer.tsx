...
export default function CleaningStrainer() {
  ...
  const deleteStrainer = async (activityId: string) => {
    try {
      await pb.collection("cleaning_strainer_ps2027").delete(activityId);
  ...
  async function loadStrainer() {
    try {
      const [activityRecords] = await Promise.all([
        pb.collection("cleaning_strainer_ps2027").getFullList({ sort: "tag_number" }), // example
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (saving) return;
      setSaving(true);
      await pb.collection("cleaning_strainer_ps2027").create(data);
  ...
  const updateField = async (
    ...
    try {
      await pb.collection("cleaning_strainer_ps2027").update(rowID, {
        [field]: value,
      });
  ...
  const handleProgressBlur = async (id: string, newProgress: number) => {
    try {
      ...
      await pb.collection("cleaning_strainer_ps2027").update(id, {
